...
To fix these inconsistencies, a numeric value in string data type for the problem track must first be assigned. The .loc() attribute can be used to call this. In this case, it shall be assigned to 0 as it is effectively unknown in the context of this data set and doing so will place it at the bottom when the set is sorted by the number of streams:
...
.head(5) can then be used to get the top 5 artists with the most written songs in the data set:
...
The graph below then shows that the data set contains more tracks that were written by solo or duo artists.
...
## Platform Popularity
The data set can be futher examined by looking at how they favor the songs in terms of their inclusion in their playlists. This can be done by simply calling on the necessary columns using .loc(), which in this case would be the track name, their artists, their playlist count and their chart rank. This is then sorted by their playlist inclusion count to check which platform favors what songs. This is done for both spotify and apple music.

### Results:
Based on the given results, it can be observed that spotify has a larger playlist inclusion count compared to apple. This suggest that spotify caters to a larger audience as oppossed to apple music. Moreover, spotify appears to favor the songs that are in their top 100 charts in terms of playlist inclusion as opposed to apple's that tends to favor the low ranking songs.

![{9AFF166C-C42C-4D12-BDD2-C99E9A2D799A}](https://github.com/user-attachments/assets/40dfb63f-3753-44a4-b2b0-ce01f027e16e)
...
![{F095A578-71EC-4835-8FE6-04F731B4326C}](https://github.com/user-attachments/assets/83941e4c-aa11-4e33-a15d-7fac4212fff4)

Songs that were written on the Major scale also has a slight advantage over those that were written in a Minor scale. Generally, Major scale songs are often the upbeat and lively melodies compared to songs that are written in the Minor scale which are often sentimental and slow. This coincides with the findings when examining the temporal trends wherein songs that have more energy and has the adequate speed of BPM tended to fair better.
